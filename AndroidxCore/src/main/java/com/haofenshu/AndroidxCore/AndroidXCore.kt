...
import com.haofenshu.lnkscreen.MyDeviceAdminReceiver
...
class AndroidXCore : Application() {
     ...
     */
    fun startLockTaskIfNeeded(context: Context) {
        try {
            if (KioskUtils.isDeviceOwner(context)) {
                ...
                // 如果不是设备所有者，尝试基础模式
                enableBasicLockTaskMode(context)
            }
        } catch (e: Exception) {
     ...
     * 基础锁定任务模式（备用方案）
     */
    private fun enableBasicLockTaskMode(context: Context) {
        try {
            ...
            dpm.setLockTaskPackages(admin, arrayOf<String?>(BuildConfig.APPLICATION_ID))

            // 检查是否允许进入LockTask模式，如果允许则启动
            if (dpm.isLockTaskPermitted(BuildConfig.APPLICATION_ID)) {
                if (context is Activity) {
                    context.startLockTask()
